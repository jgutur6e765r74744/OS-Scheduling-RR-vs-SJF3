...
Results are displayed through tables and Gantt charts for visualization, along with a PDF report that summarizes the comparison between both algorithmsز

2:Project Requirements • Python 3.x • Tkinter (GUI) • Matplotlib (Charts + Gantt) • FPDF (PDF Report) • deque (for Round Robin queue)

Functional Requirements: • Add processes via GUI • Load test scenarios • Validate input data • Run Round Robin & Preemptive SJF • Display Gantt charts and metrics (WT, TAT, RT) • Show comparison results • Export PDF report 3: team members:
